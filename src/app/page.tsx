...
function Plant({lang}:{lang:string}){const he=lang==="he";const facts=[
  ...
  <p style={{fontSize:14,color:P.steel,lineHeight:1.9,marginBottom:16}}>{he?"פרויקט הגז ברזאן מיועד לטפל בגז גולמי המופק מהשדה הצפוני של קטר, במטרה לספק גז איכותי לרשת החשמל ולהתפלת המים המקומית. בשונה ממתקני ייצוא ה-LNG הסמוכים, מתקן זה מבצע הפרדה וזיקוק בלחצים גבוהים של מספר קונבנסיציות פחמימניות ורכיבים נלווים, ומכאן רגישותו לאירועי כשל.":"The Barzan project processes raw gas from Qatar's North Field to supply high-quality gas to the domestic power grid and water desalination. Unlike the adjacent LNG export trains, this plant performs high-pressure separation and refining of several hydrocarbon condensates and associated components — hence its sensitivity to failure events."}</p>
  {/* Facts grid */}
...
const stages=[
  ...
  {id:"frac",ic:"⚗️",x:64,lb:he?"הפרדת NGL":"NGL Fractionation",c:"#8b5cf6",
   d:he?"הנוזלים הטבעיים (NGL) — אתאן, פרופאן, בוטאן — מופרדים בעמודות פרקציה בלחצים שונים. כל רכיב מנוזל בנקודת רתיחה אחרת. אלה הנוזלים שבלחץ גבוה התנדפו במהירות ויצרו את ענן האדים באירוע.":"Natural gas liquids (NGL) — ethane, propane, butane — are separated in fractionation columns at varying pressures. Each component condenses at a different boiling point. These are the liquids that, at high pressure, flashed rapidly to form the vapor cloud in the incident."},
...
function Timeline({lang}:{lang:string}){const he=lang==="he";const ev=[
  ...
  {t:"03/2026",h:he?"מתקפת טילים איראנית פוגעת בראס לאפן — 2 רכבות LNG ומתקן GTL ניזוקים (~17% מכושר היצוא). נזק אזורי למערכות מבניות בצנרת":"Iranian missile strike hits Ras Laffan — 2 LNG trains & a GTL facility damaged (~17% of export capacity). Regional structural damage to piping systems",c:P.red,r:true},
...
const phases=[
  ...
  {id:1,lb:he?"2. ענן אדים":"2. Vapor Cloud",d:he?"הגז מתאדה ומתפשט, נדל לפי הרוח ומגיע לטווח נפיצות (LEL-UEL)":"Gas vaporizes and spreads, drifts with wind, reaching flammable range (LEL-UEL)",c:P.red},
...
return<Sec id="vce" num="06" title={he?"הדמיית פיצוץ ענן האדים (VCE)":"Vapor Cloud Explosion (VCE) Simulation"} subtitle={he?"חמשת השלבים — אנימציה מתמשכת":"The five stages — continuous animation"} dark sidebar={<>
  ...
  <SB color="red" title={he?"BLEVE — סיכון משני":"BLEVE — Secondary Risk"}><p>{he?"Boiling Liquid Expanding Vapor Explosion: אם להבות פוגעות במיכל גז מנוזל תחת לחץ, הרתחת הנוזל הכלוא עלולה לקרוע את המיכל בפיצוץ אלים. סיכון מתמשך כל עוד יש זרימת דלק.":"Boiling Liquid Expanding Vapor Explosion: if flames impinge a pressurized liquefied-gas vessel, the boiling confined liquid can rupture the vessel in a violent explosion. An ongoing risk while fuel flows."}</p></SB>
</>}>
...
const mats:Record<string,any>={
  ...
  ngl:{name:he?"נוזלי גז טבעי":"Natural Gas Liquids",formula:"C₂–C₄",cas:"—",un:"1075",icon:"💧",c:P.amber,
    ...
    props:[["LEL","~2%"],["UEL","~10%"],[he?"רעילות":"Toxicity",he?"מחניק":"Asphyxiant"],[he?"צפיפות יחסית":"Rel. density",">אוויר"],[he?"מצב":"State",he?"גז מנוזל":"Liquefied gas"],[he?"סיכון":"Hazard","BLEVE"]],
    pac:[{l:"PAC-1 (TEEL-1)",v:"65,000 ppm",d:he?"דחיקת חמצן ל-19.5%":"O₂ down to 19.5%",c:"#fbbf24"},{l:"PAC-2 (TEEL-2)",v:"230,000 ppm",d:he?"דחיקת חמצן ל-16%":"O₂ down to 16%",c:"#f59e0b"},{l:"PAC-3 (TEEL-3)",v:"400,000 ppm",d:he?"דחיקת חמצן ל-12.5%":"O₂ down to 12.5%",c:"#dc2626"}],
...
function Response({lang}:{lang:string}){const he=lang==="he";const cards=[
  ...
  {ic:"🛡️",ti:he?"אזורי סיכון ובחירת מיגון":"Risk Zones & PPE Selection",c:P.purple,
   pts:he?["בשלבים הראשוניים, עקב החשש מנוכחות H₂S חופשי באוויר, הוגדר אזור הלחימה המיידי כאזור חם המחייב מיגון נשימתי ועורי מרבי.","מעבר לחליפות Level B: צוותי הסריקה והחילוץ הונחו לפעול בחליפות מגן מפני נוזלים/נתזים בשילוב מערכות נשימה פתוחות (מנ\"פ). רמת מיגון זו נבחרה מכיוון שהסיכון הדומיננטי בשלב זה אינו ענן גז פחממני דליק מתפשט, אלא פליטות מקומיות וכיסים כלואים של גז רעיל (H₂S).","עם דעיכת הלהבות ומעבר הזירה לשלב סריקה וחילוץ הריסות, בוצעה הערכת סיכונים מחודשת."]:["In initial stages, due to concern over free H₂S in the air, the immediate combat zone was defined as a hot zone requiring maximum respiratory and skin protection.","Transition to Level B suits: search and rescue teams were directed to operate in liquid/splash protective suits combined with open breathing systems (SCBA). This protection level was chosen because the dominant hazard at this stage is not a spreading flammable hydrocarbon cloud, but localized releases and confined pockets of toxic gas (H₂S).","As flames subsided and the scene transitioned to search and debris extraction, a renewed risk assessment was conducted."]},
  ...
  <SB color="blue" title={he?"רמות מיגון":"Protection Levels"}><p style={{marginBottom:4}}><b>Level A</b> — {he?"אטימה מלאה + מנ\"פ":"Full encapsulation + SCBA"}</p><p style={{marginBottom:4}}><b>Level B</b> — {he?"מנ\"פ + הגנת נתזים":"SCBA + splash protection"}</p><p>{he?"בברזאן: Level B עקב סיכון רעיל מקומי, לא ענן מתפשט.":"At Barzan: Level B due to localized toxic risk, not a spreading cloud."}</p></SB>
</>}>
...
function Geo({lang}:{lang:string}){const he=lang==="he";return<Sec id="geo" num="10" title={he?"רקע גאופוליטי":"Geopolitical Context"} subtitle={he?"מדוע המתקן היה בתהליך התנעה רגיש":"Why the plant was in a sensitive restart"} dark sidebar={<>
  <SB color="amber" title={he?"מצר הורמוז":"Strait of Hormuz"}><p>{he?"כמעט כל תוצרת ראס לאפن עוברת דרך מצר הורמוז. סגירתו בתקופת המלחמה שיתקה את היצוא הקטרי והאיצה את הלחץ להתנעה מחדש.":"Almost all Ras Laffan output passes through the Strait of Hormuz. Its closure during the war paralyzed Qatari exports and accelerated pressure for restart."}</p></SB>
  <SB color="red" title={he?"17% מכושר היצוא":"17% of Export Capacity"}><p>{he?"מתקפת מרץ 2026 הסירה כ-17% מכושר ייצוא ה-LNG של קטר — 2 רכבות LNG ומתקן GTL. הלחץ לחזור לייצור יצר את חלון ההתנעה המסוכן.":"The March 2026 strike removed ~17% of Qatar's LNG export capacity — 2 LNG trains and a GTL facility. Pressure to resume production created the dangerous restart window."}</p></SB>
</>}>
  <p style={{fontSize:14,color:P.steel,lineHeight:1.9,marginBottom:16}}>{he?"אירוע ברזאן אינו מתקיים בחלל ריק. הוא מגיע ברגע שבו תשתית האנרגיה הקטרית כבר פועלת תחת לחץ משמעותי. בחודש מרץ 2026, במהלך העימות בין ארה\"ב-ישראל לאיראן, מתקפת טילים איראנית פגעה בקריית התעשייה ראס לאפן — בתגובה למדינות המארחות בסיסים אמריקאיים. המתקפה גרמה נזק לשתיים מתוך 14 רכבות ה-LNG ולאחד משני מתקני ה-GTL.":"The Barzan event does not exist in a vacuum. It arrives at a moment when Qatari energy infrastructure is already operating under significant strain. In March 2026, during the US-Israel conflict with Iran, an Iranian missile strike hit Ras Laffan Industrial City — in response to countries hosting US bases. The strike damaged two of the 14 LNG trains and one of the two GTL facilities."}</p>
...
function Lessons({lang}:{lang:string}){const he=lang==="he";const lessons=[
  {n:"1",ic:"👁️",ti:he?"חיוניות של תאומים דיגיטליים":"Digital Twins are Vital",c:P.blue,
   d:he?"אירוע זה מוכיח את הצורך בהטמעת מערכות סימולציה מבוססות בינה מלאכותית, אשר מסוגלות להצליב נתוני SCADA בזמן אמת עם מכני ברקמות הצינור הצפוי, כדי להתריע על חריגות לחץ זעירות לפני התרחשות כשל בשלב התנעה (Start-up).":"This event proves the need for AI-based simulation systems that can cross-reference real-time SCADA data with expected pipe-network mechanics, to alert on tiny pressure anomalies before a failure occurs during start-up."},
  ...
  {y:"1944",n:"Cleveland",l:"USA",d:he?"כשל מיכל אחסון LNG. דליפה לביוב העירוני, התאדות ופיצוץ. 130 הרוגים. אחד האסונות שעיצבו את תקני אחסון הגז המנוזל.":"LNG storage tank failure. Leak into city sewers, vaporization and explosion. 130 dead. One of the disasters that shaped liquefied-gas storage standards."},
